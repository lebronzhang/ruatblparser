...
class ExtendedKalmanFilterSLAM:
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        state = zeros(self.state.size + 2)
        state[0:self.state.size] = self.state
        state[self.state.size] = initial_coords[0]
        state[self.state.size + 1] = initial_coords[1]
        self.number_of_landmarks += 1

        covariance = zeros((state.size, state.size))
        covariance[0:self.state.size,0:self.state.size] = self.covariance
        covariance[self.state.size, self.state.size] = 1e10
        covariance[self.state.size + 1, self.state.size + 1] = 1e10
